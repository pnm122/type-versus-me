...
const readableErrorReasons: Record<ErrorReason, string | null> = {
  ...
  'invalid-username': 'Your username must be between 3 and 16 characters (alphanumeric or underscore).',
...
export function errorNotification(reason: ErrorReason): NotificationData {
  return {
    style: 'error',
    text: readableErrorReasons[reason] ?? `Something went wrong! Please refresh and try again. (Error code: ${reason})`
  }
}